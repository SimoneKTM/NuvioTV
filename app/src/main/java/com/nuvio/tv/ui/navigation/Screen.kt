...
sealed class Screen(val route: String) {
    ...
    data object Extra : Screen("extra")
    data object ExtraSettings : Screen("extra_settings")
    ...
    data object CatalogSeeAll : Screen("catalog_see_all/{catalogId}/{addonId}/{type}?fromSearch={fromSearch}&fromAnime={fromAnime}&fromExtra={fromExtra}") {
        private fun encode(value: String): String =
            URLEncoder.encode(value, "UTF-8").replace("+", "%20")

        fun createRoute(catalogId: String, addonId: String, type: String, fromSearch: Boolean = false, fromAnime: Boolean = false, fromExtra: Boolean = false): String {
            return "catalog_see_all/${encode(catalogId)}/${encode(addonId)}/${encode(type)}?fromSearch=$fromSearch&fromAnime=$fromAnime&fromExtra=$fromExtra"
        }
    }
